...
(function ($, exports) {
    ...
    vizz.version = '2.0.0';
    //是否数组
    var isArray = vizz.isArray = function (obj) {
        return Object.prototype.toString.call(obj) === '[object Array]';
    };
    //是否对象
    var isObject = vizz.isObject = function (obj) {
        return Object.prototype.toString.call(obj) === '[object Object]';
    };
    //集合转化成数组
    var makeArray = vizz.makeArray = function (args) {
    ...
    //vizz ajax模块，在jQuery基础上扩展适合域思自身业务的ajax
    var _ajax = vizz.ajax = function (options) {
        var setting = {
            url: '',
            data: null,
            crossdomain: false,
    ...
    var Ajax = {
        ajax: function (options) {
            options.success = this.proxy(options.success || function () {
            });
            options.error = this.proxy(options.error || function () {
            });
            options.complete = this.proxy(options.complete || function () {
            });
            _ajax(options);
        },
        getRemote: function (url, callback) {
            this.ajax({
                type: 'GET',
                url: url,
                success: this.proxy(callback)
            });
        },
        postRemote: function (url, data, callback) {
            this.ajax({
                type: 'POST',
                url: url,
                data: data,
        ...
        //利用crossdomain.ashx作代理的跨域请求，使用前确保网站根目录存在crossdomain.ashx文件
        cross: function (url, data, callback) {
            this.ajax({
                url: url,
                crossdomain: true,
                data: data,
        ...
        //以 .net webservice 的方式发请求
        webservice: function (url, data, callback) {
            this.ajax({
                url: url,
                webservice: true,
                data: data,
    ...
    //定义模型
    var Model = vizz.Model = Class.create();
    //在Model添加事件模块
    Model.extend(Events);
    //添加Model 类的属性
    Model.extend({
        ...
        refresh: function (values) {
            ...
            this.trigger("refresh", values, this.records);
        },
        load: function (values) {
            ...
            this.trigger("load", values, this.records);
        },
        ...
        findOne: function (name, value) {
            for (var key in this.records) {
                if (this.records[key].attributes[name] == value) {
                    return this.records[key].clone();
                }
            }
        },
        findAll: function (name, value) {
    ...
    Model.include({
        ...
        set: function (nameOrAttr, value) {
            if (nameOrAttr && isObject(nameOrAttr)) {
                this.load(nameOrAttr);
                return this;
            }
            else
                if (nameOrAttr && value) {
                    this.attributes[nameOrAttr] = value;
                    return this;
                }
        },
        destroy: function () {
        ...
        dup: function () {
            var result = this.parent.init(this.get());
    ...
    //在Model和Model的实例都添加Ajax模块
    Model.extend(Ajax);
    Model.include(Ajax);
    //事件分隔符，在控制器中使用
    var eventSplitter = /^(\w+)\s*(.*)$/;
    //定义控制器
    var Controller = vizz.Controller = Class.create({
        tag: "div",
        //控制器初始化
    ...
    //html解码
    var decodeHTML = vizz.decodeHTML = function (html) {
    ...
    //html转码
    var encodeHTML = vizz.encodeHTML = function (html) {
        return String(html).replace(/["<>& ]/g, function (all) {
            return "&" + _htmlEncodeDict[all] + ";";
        });
    };
    //清除html标记
    var clearHTML = vizz.clearHTML = function (html) {
        html = html.replace(/<\/?[^>]*>/g, '');
        html = html.replace(/[ | ]*\n/g, '\n');
        html = html.replace(/\n[\s| | ]*\r/g, '\n');
    ...
    var replaceWith = vizz.replaceWith = function (template, obj) {
        var _this = this;
    ...
    //查询字符串转换成json
    vizz.queryToJson = function (url, isDecode) {
    ...
    //获取Url参数
    vizz.getParam = vizz.request = function (name) {
        var json = vizz.queryToJson();
        if (name) {
            return json[name];
        } else {
            return json;
    ...
    //获取表单元素值，并转换成json
    vizz.getForm = function (wrapper) {
        var query = $('input,select,textarea', $(wrapper || exports.document)).serialize();
        return this.queryToJson(query, true);
    };
    //设置表单元素值
    vizz.setForm = function (json, wrapper) {
    ...
    vizz.charCount = function (str) {
        return str.replace(/[^\x00-\xff]/g, "xx").length;
    };

    //截取字符串
    vizz.cutStr = function (str, len) {
        var str_length = 0;
        var str_len = 0;
        str_cut = new String();
        str_len = str.length;
    ...
    //日期函数
    vizz.Date = function (dateStr, format, options) {
        if (!dateStr) {
            return ("");
        };
        var obj = null;
    ...
    //定义命名空间
    vizz.namespace = vizz.package = function (d) {
        var c = window;
        var e = d.split(".");
    ...
    vizz.isDigits = function (value) {
        return /^\d+$/.test(value);
    };
    vizz.isNumber = function (value) {
        return /^-?(?:\d+|\d{1,3}(?:,\d{3})+)?(?:\.\d+)?$/.test(value);
    };
    vizz.isDate = function (value) {
        return !/Invalid|NaN/.test(new Date(value));
    };

    vizz.isMobile = function (value) {
        return /^1[358][0-9]{9}$/.test(value);
    };
    vizz.isPhone = function (value) {
        return /^[0-9\-\—]{6,20}$/.test(value);
    };


    //导出全局变量
    exports.vizz = exports.vizz = vizz;
})(jQuery, window);